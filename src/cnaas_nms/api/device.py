...
device_initcheck_model = device_initcheck_api.model(
    "device_initcheck", {"hostname": fields.String(required=False), "device_type": fields.String(required=False)}
)
...
class DeviceInitCheckApi(Resource):
    ...
    def post(self, device_id: int):
        ...
        with sqla_session() as session:
            ...
            if mlag_peer_id:
                try:
                    mlag_peer_dev = cnaas_nms.confpush.init_device.pre_init_checks(session, mlag_peer_id)
                except ValueError as e:
                    return empty_result(status="error", data="ValueError in pre_init_checks: {}".format(e)), 400
                except Exception as e:
                    return empty_result(status="error", data="Exception in pre_init_checks: {}".format(e)), 500
            ...
            try:
                ret["linknets"] = cnaas_nms.confpush.update.update_linknets(
                    session, hostname=dev.hostname, devtype=target_devtype, ztp_hostname=target_hostname, dry_run=True
                )
                if mlag_peer_dev:
                    ret["linknets"] += cnaas_nms.confpush.update.update_linknets(
                        session,
