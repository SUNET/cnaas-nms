...
def limit_results() -> int:
    """Find number of results to limit query to, either by user requested param or a default value."""
...
def offset_results() -> int:
    """Find number of results to offset query to, either by user requested param or a default value."""
    offset = 0
...
def build_filter(f_class, query: sqlalchemy.orm.query.Query):
    """
    Generate SQLalchemy filter based on query string and return filtered query.
    ...
    for arg, value in args.items():
        ...
        if isinstance(f_class.__table__._columns[attribute].type, sqlalchemy.Enum):
            value = value.upper()
            allowed_names = {item.name for item in f_class.__table__._columns[attribute].type.enum_class}
        ...
        if operator == "contains":
            ...
            f_class_op = f_class_field.ilike
            value = "%" + value + "%"
        else:
            f_class_op = f_class_field.__eq__
    ...
    if f_class_order_by_field:
        ...
        if "id" in f_class.__table__._columns.keys():
            order = sqlalchemy.asc
            f_class_order_by_field = f_class.id
            query = query.order_by(order(f_class_order_by_field))
...
def parse_pydantic_error(e: Exception, schema, data: dict) -> List[str]:
    errors = []
    for _, error in enumerate(e.errors()):
        loc = error["loc"]
        errors.append(
