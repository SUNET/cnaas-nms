    ...
    def post(self):
        ...
        with sqla_session() as session:
            dev_a: Device = session.query(Device).\
                filter(Device.hostname == json_data['device_a']).one_or_none()
            if not dev_a:
                return empty_result(
                    status='error',
                    data="Hostname '{}' not found".format(json_data['device_a'])
                ), 500

            dev_b: Device = session.query(Device). \
                filter(Device.hostname == json_data['device_b']).one_or_none()
            if not dev_b:
                return empty_result(
                    status='error',
                    data="Hostname '{}' not found".format(json_data['device_b'])
                ), 500
