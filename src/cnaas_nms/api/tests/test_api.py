...
class ApiTests(unittest.TestCase):
    ...
    def test_get_single_device(self):
        ...
        result = self.client.get("/api/v1.0/devices", query_string={"filter[hostname]": hostname})
    ...
    def test_filter_job(self):
        ...
        self.assertEqual(len(first_device_result["job_tasks"]), 2, "Job result output should only contain 2 tasks")
        self.assertEqual(
            len(list(filter(lambda x: x["task_name"] != "Generate device config", first_device_result["job_tasks"]))),
            2,
    ...
    def test_get_interfaces(self):
        result = self.client.get("/api/v1.0/device/{}/interfaces".format(self.testdata["interface_device"]))
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
    ...
    def test_update_interface_configtype(self):
        ...
        result = self.client.put("/api/v1.0/device/{}/interfaces".format(self.testdata["interface_device"]), json=data)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
    ...
    def test_update_interface_data_untagged(self):
        # Test untagged_vlan
        ifname = self.testdata["interface_update"]
        data = {"interfaces": {ifname: {"data": {"untagged_vlan": self.testdata["untagged_vlan"]}}}}
        result = self.client.put("/api/v1.0/device/{}/interfaces".format(self.testdata["interface_device"]), json=data)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
        #        self.assertEqual(ifname in result.json['data']['updated'], True)
    ...
    def test_update_interface_data_tagged(self):
        # Test tagged
        ifname = self.testdata["interface_update"]
        data = {"interfaces": {ifname: {"data": {"tagged_vlan_list": self.testdata["tagged_vlan_list"]}}}}
        result = self.client.put("/api/v1.0/device/{}/interfaces".format(self.testdata["interface_device"]), json=data)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
        #        self.assertEqual(ifname in result.json['data']['updated'], True)
    ...
    def equipmenttest_get_interface_status(self):
        result = self.client.get("/api/v1.0/device/{}/interface_status".format(self.testdata["interface_device"]))
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
    ...
    def equipmenttest_bounce_interface(self):
        ...
        result = self.client.put(
            "/api/v1.0/device/{}/interface_status".format(self.testdata["interface_device"]), json=data
        )
        self.assertEqual(result.status_code, 400)
        self.assertEqual(result.json["status"], "error")
    ...
    def test_get_groups(self):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
        self.assertTrue(groupname in result.json["data"]["groups"], f"Group '{groupname}' not found")
        self.assertGreaterEqual(
            len(result.json["data"]["groups"][groupname]), 1, f"No devices found in group '{groupname}'"
        )
    ...
    def test_get_groups_osversion(self):
        ...
        self.assertEqual(result.json["status"], "success")
        self.assertGreaterEqual(
            len(result.json["data"]["groups"][groupname]), 1, f"No devices found in group '{groupname}' os_versions"
        )
    ...
    def test_generate_only_vars(self):
        result = self.client.get("/api/v1.0/device/{}/generate_config".format(self.testdata["interface_device"]))
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json["status"], "success")
        self.assertEqual(
    ...
    def test_linknet(self):
        ...
        self.assertEqual(result.json["status"], "success")
        self.assertGreaterEqual(len(result.json["data"]["linknets"]), 1, "Less than one linknet found on GET linknets")
