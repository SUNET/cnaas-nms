...
def test_get_single_device(testdata, client):
    hostname = testdata["managed_dist"]
    result = client.get("/api/v1.0/devices", query_string={"filter[hostname]": hostname})
    print(result.json)
...
def test_filter_job(client):
    result = client.get("/api/v1.0/jobs?filter[function.name][contains]=sync&filter_jobresult=config")
    print(result.json)
    ...
    assert len(first_device_result["job_tasks"]) == 2, "Job result output should only contain 2 tasks"
    assert (
        len(
            list(
                filter(
...
def test_update_interface_data_neighbor(client, testdata):
    ifname = testdata["interface_update"]
    data = {"interfaces": {ifname: {"data": {"neighbor_id": 123, "neighbor": "testhostname"}}}}
...
def test_get_managementdomain(client):
...
def test_update_managementdomain(client):
    result = client.get("/api/v1.0/mgmtdomains?per_page=1")
    assert isinstance(result.json["data"]["mgmtdomains"][0]["id"], int)
...
def test_validate_managementdomain(client):
    result = client.get("/api/v1.0/mgmtdomains?per_page=1")
    assert isinstance(result.json["data"]["mgmtdomains"][0]["id"], int)
    id = result.json["data"]["mgmtdomains"][0]["id"]
...
def test_repository_refresh(client):
    data = {"action": "refresh"}
...
def test_repository_get(client):
...
def test_get_interfaces(client, testdata):
    result = client.get("/api/v1.0/device/{}/interfaces".format(testdata["interface_device"]))
    assert result.status_code == 200
    assert result.json["status"] == "success"


def test_update_interface_configtype(client, testdata):
    ifname = testdata["interface_update"]
...
def test_update_interface_data_tagged(client, testdata):
    # Test tagged
    ifname = testdata["interface_update"]
    data = {"interfaces": {ifname: {"data": {"tagged_vlan_list": testdata["tagged_vlan_list"]}}}}
    result = client.put("/api/v1.0/device/{}/interfaces".format(testdata["interface_device"]), json=data)
    assert result.status_code == 200
    assert result.json["status"] == "success"
    #        self.assertEqual(ifname in result.json['data']['updated'], True)
...
def test_update_interface_data_enabled(client, testdata):
    # Disable interface
    ifname = testdata["interface_update"]
...
def test_get_interface_status(client, testdata):
    result = client.get("/api/v1.0/device/{}/interface_status".format(testdata["interface_device"]))
    assert result.status_code == 200
    assert result.json["status"] == "success"
...
@pytest.mark.equipment
def test_bounce_interface(client, testdata):
...
def test_get_groups(client, testdata):
    ...
    assert len(result.json["data"]["groups"][groupname]) >= 1, f"No devices found in group '{groupname}'"


def test_get_groups_osversion(client, testdata):
    groupname = testdata["groupname"]
    result = client.get(f"/api/v1.0/groups/{groupname}/os_version")
    assert result.status_code == 200
    assert result.json["status"] == "success"
    assert len(result.json["data"]["groups"][groupname]) >= 1, f"No devices found in group '{groupname}' os_versions"


def test_renew_cert_errors(client, testdata):
    # Test invalid hostname
...
def test_generate_only_vars(client, testdata, templates_directory):
...
def test_linknet(client):
    ...
    assert result.json["status"] == "success"
    assert len(result.json["data"]["linknets"]) >= 1, "Less than one linknet found on GET linknets"
