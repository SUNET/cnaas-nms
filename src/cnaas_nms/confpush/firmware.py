...
def device_upgrade_task(
    ...
    if reboot and not already_active:
        ...
        try:
            task.run(task=arista_device_reboot, job_id=job_id)
...
def device_upgrade(
    ...
    logger.info(
        f"Upgrade tasks selected: pre_flight = {pre_flight}, download = {download}, activate = {activate}, reboot = {reboot}, post_flight = {post_flight}"
    )
