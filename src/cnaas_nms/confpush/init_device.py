...
def push_base_management(task, device_variables: dict, devtype: DeviceType, job_id):
    ...
    # TODO: handle exception from ztp_device_cert -> arista_copy_cert
        ...
        logger.exception("Unable to install copy the certificate.")
    else:
        if device_cert_res.failed:
            if api_settings.VERIFY_TLS_DEVICE:
                logger.error("Unable to install device certificate for {}, aborting".format(device_variables["host"]))
                raise Exception(device_cert_res[0].exception)
            else:
                logger.debug("Unable to install device certificate for {}".format(device_variables["host"]))
...
def pre_init_checks(session, device_id) -> Device:
    """Find device with device_id and check that it's ready for init, returns device object or raises exception."""
...
def init_access_device_step1(
    ...
    uplink_hostnames_arg: Optional[List[str]] = None,
    ...
    Args:
        ...
        uplink_hostnames_arg: List of hostnames of uplink peer devices (optional) Used when initializing MLAG peer device
    ...
    """
    if uplink_hostnames_arg is None:
    ...
    with sqla_session() as session:
        ...
        if mlag_peer_id and mlag_peer_new_hostname:
            ...
            update_interfacedb_worker(
                session, dev, replace=True, delete_all=False, mlag_peer_hostname=mlag_peer_dev.hostname
            )
            update_interfacedb_worker(
                session, mlag_peer_dev, replace=True, delete_all=False, mlag_peer_hostname=dev.hostname
            )
...
def discover_device(
    ...
    try:
        ...
        with sqla_session() as session:
            ...
            logger.info(
                f"Device with ztp_mac {ztp_mac} successfully scanned (attempt {iteration}), moving to DISCOVERED state"
            )
    except Exception as e:
