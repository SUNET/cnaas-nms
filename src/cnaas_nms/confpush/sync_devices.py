...
def generate_asn(ipv4_address: IPv4Address) -> Optional[int]:
    """Generate a unique private 4 byte AS number based on last two octets of an IPv4 address (infra_lo)."""
...
def resolve_vlanid(vlan_name: str, vxlans: dict) -> Optional[int]:
    ...
    if not isinstance(vlan_name, str):
        return None
    for _, vxlan_data in vxlans.items():
        try:
...
def populate_device_vars(
    ...
    if devtype == DeviceType.ACCESS:
            ...
            for intf in settings["interfaces"]:
                ...
                if intf["ifclass"] == "downlink":
                    ...
                    fabric_device_variables["interfaces"].append(
                        {"name": intf["name"], "ifclass": intf["ifclass"], "indexnum": ifindexnum, "data": data}
                    )
        ...
        for local_if, _ in fabric_interfaces.items():
            logger.warning(
...
def sync_devices(
    ...
    """
    Synchronize devices to their respective templates.

    If no arguments are specified then synchronize all devices that are currently out of sync.
    ...
    Args:
        ...
        force: Commit config even if changes made outside CNaaS will get overwritten
        ...
        resync: Re-synchronize a device even if it's marked as synced in the database, a device selected by hostname is always re-synced
