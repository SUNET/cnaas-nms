        ...
        with sqla_session() as session:
            ...
            for hostname in updated_hostnames:
                ...
                if dev:
                    ...
                    logger.warning("Settings updated for unknown device: {}".format(hostname))
...
def template_syncstatus(updated_templates: set) -> Set[Tuple[DeviceType, str]]:
    """Determine what device types have become unsynchronized because of updated template files."""
    ...
    for platform in os.listdir(local_repo_path):
        ...
        for devtype in DeviceType:
            if devtype.name in mapping:
                ...
                try:
                    dependencies = [mapping[devtype.name]["entrypoint"]]
...
def settings_syncstatus(updated_settings: set) -> Tuple[Set[DeviceType], Set[str]]:
    """Determine what devices has become unsynchronized after updating the settings repository."""
    unsynced_devtypes = set()
