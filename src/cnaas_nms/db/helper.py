...
def canonical_mac(mac):
    """Return a standardized format of MAC-addresses for CNaaS to store in databases etc."""
...
def find_mgmtdomain(session, hostnames: List[str]) -> Optional[Mgmtdomain]:
    """
    Find the corresponding management domain for a pair of distribution switches.
    ...
    if not isinstance(hostnames, list) or not len(hostnames) == 2:
        raise ValueError("Two uplink devices are required to find a compatible mgmtdomain, got: {}".format(hostnames))
