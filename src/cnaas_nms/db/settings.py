...
def model_name_sanitize(model_name: str):
    """Return the model name sanitized for filename purposes, strip whitespace, convert to lowercase etc."""
...
def get_setting_filename(repo_root: str, path: List[str]) -> str:
    """
    Check that the setting filename is defined in DIR_STRUCTURE and if so return a proper os path to the setting file.
...
def get_pydantic_field_descr(schema: dict, loc: tuple):
    """Get the description from a pydantic Field definition based on a model schema and a "loc" tuple from pydantic ValidatorError.errors()."""
    next_schema = None
    for loc_part in loc:
    ...
    try:
        ...
        for num, error in enumerate(validation_error.errors()):
            ...
            try:
                ...
                logger.debug(str(descr_error))  # noqa: G200
...
def filter_yamldata(data: Union[List, dict], groups: List[str], hostname: str, recdepth=100) -> Union[List, dict]:
    """
    Filter data and remove dictionary items.

    If they have a key that specifies a list of groups, but none of those groups are included in the groups argument.
...
def get_settings(
    ...
    """Get settings to use for device matching hostname or global settings if no hostname is specified."""
    logger = get_logger()
    ...
    if diff_model:
        logger.warning("Some configured settings are undefined in model: {}".format(set_everything - set_model))
...
def get_group_regex(group_name: str) -> Optional[str]:
    """Return a string containing the regex defining the specified group name if it's found."""
    settings, origin = get_group_settings()
    if not settings:
        return None
    if not settings.get("groups", None):
...
def parse_device_primary_groups() -> Dict[str, str]:
    """Return a dict with {hostname: primary_group} from settings."""
...
def get_device_primary_groups(no_cache: bool = False) -> Dict[str, str]:
    """
    Return a dict with {hostname: primary_group} from redis.
