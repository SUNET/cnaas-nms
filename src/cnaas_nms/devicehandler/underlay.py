...
def find_free_infra_ip(session) -> Optional[IPv4Address]:
    ...
    used_ips = []
    device_query = session.query(Device).filter(Device.infra_ip != None).options(load_only("infra_ip"))  # noqa: E711
...
def find_free_mgmt_lo_ip(session) -> Optional[IPv4Address]:
    ...
    reserved_ips = []
    device_query = (
        session.query(Device).filter(Device.management_ip != None).options(load_only("management_ip"))  # noqa: E711
    )
    for device in device_query:
...
def find_free_infra_linknet(session) -> Optional[IPv4Network]:
    ...
    used_linknets = []
    linknet_query = session.query(Linknet).filter(Linknet.device_a_ip != None)  # noqa: E711
    ln: Linknet
