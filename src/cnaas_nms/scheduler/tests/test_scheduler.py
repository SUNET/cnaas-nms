...
class InitTests(unittest.TestCase):
    ...
    def test_add_schedule(self):
        ...
        job1_id = scheduler.add_onetime_job(
            testfunc_success, when=1, scheduled_by="test_user", kwargs={"text": "success"}
        )
        job2_id = scheduler.add_onetime_job(
            testfunc_exception, when=1, scheduled_by="test_user", kwargs={"text": "exception"}
        )
        assert isinstance(job1_id, int)  # noqa: T001
        assert isinstance(job2_id, int)  # noqa: T001
        print(f"Test job 1 scheduled as ID { job1_id }")  # noqa: T001
        print(f"Test job 2 scheduled as ID { job2_id }")  # noqa: T001
        time.sleep(3)
    ...
    def test_abort_schedule(self):
        ...
        job3_id = scheduler.add_onetime_job(
            testfunc_success, when=600, scheduled_by="test_user", kwargs={"text": "abort"}
        )
        assert isinstance(job3_id, int)
        print(f"Test job 3 scheduled as ID { job3_id }")  # noqa: T001
