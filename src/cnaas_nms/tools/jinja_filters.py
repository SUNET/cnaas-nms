...
def template_filter(name: Optional[str] = None) -> Callable:
    """
    Register a template filter function in the FILTERS dict.
...
def ipv4_to_ipv6(v6_network: Union[str, ipaddress.IPv6Network], v4_address: Union[str, ipaddress.IPv4Interface]):
    """
    Transform an IPv4 address to an IPv6 interface address.

    This will combine an arbitrary IPv6 network address with the 32 address bytes of an IPv4 address into a valid IPv6 address
...
def get_interface(
    ...
    """
    Return a host address with a prefix length from its index in a network.
