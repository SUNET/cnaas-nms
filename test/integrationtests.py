...
class GetTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        for _ in range(100):
            try:
                ...
                print("Exception {}, retrying in 1 second".format(str(e)))  # noqa: T001
                time.sleep(1)
            else:
                if r.status_code == 200:
                    return True
                else:
                    print("Bad status code {}, retrying in 1 second...".format(r.status_code))  # noqa: T001
                    time.sleep(1)
        raise AssertionError("Failed to test connection to API")
    ...
    def wait_for_discovered_device(self):
        for _ in range(100):
            ...
            if len(r.json()["data"]["devices"]) == 1:
                return r.json()["data"]["devices"][0]["hostname"], r.json()["data"]["devices"][0]["id"]
            else:
                time.sleep(5)
        return None, None
    ...
    def check_jobid(self, job_id):
        for _ in range(100):
            r = requests.get(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, verify=TLS_VERIFY)
            if r.status_code == 200:
    ...
    def test_00_sync(self):
        ...
        print("Template refresh status: {}".format(r.status_code))  # noqa: T001
        ...
        print("Settings refresh status: {}".format(r.status_code))  # noqa: T001
    ...
    def test_01_init_dist(self):
        ...
        r = requests.post(f"{URL}/api/v1.0/device", headers=AUTH_HEADER, json=new_dist_data, verify=TLS_VERIFY)
        ...
        new_mgmtdom_data = {"ipv4_gw": "10.0.6.1/24", "device_a": "eosdist1", "device_b": "eosdist2", "vlan": 600}
        r = requests.post(f"{URL}/api/v1.0/mgmtdomains", headers=AUTH_HEADER, json=new_mgmtdom_data, verify=TLS_VERIFY)
    ...
    def test_02_ztp(self):
        ...
        print("Discovered hostname, id: {}, {}".format(hostname, device_id))  # noqa: T001
        ...
        r = requests.post(f"{URL}/api/v1.0/device_init/{device_id}", headers=AUTH_HEADER, json=data, verify=TLS_VERIFY)
        ...
        self.assertTrue(result["devices"]["eosaccess"]["failed"], "Expected failed result since mgmt_ip changed")
        time.sleep(5)
        step2_job_data = self.check_jobid(step1_job_data["next_job_id"])
        result_step2 = step2_job_data["result"]
        self.assertFalse(result_step2["devices"]["eosaccess"]["failed"], "Could not reach device after ZTP")

    def test_03_interfaces(self):
        r = requests.get(f"{URL}/api/v1.0/device/eosaccess/interfaces", headers=AUTH_HEADER, verify=TLS_VERIFY)
    ...
    def test_05_update_facts_dist(self):
        hostname = "eosdist1"
        r = requests.post(
            f"{URL}/api/v1.0/device_update_facts", headers=AUTH_HEADER, json={"hostname": hostname}, verify=TLS_VERIFY
        )
    ...
    def test_07_genconfig(self):
        r = requests.get(f"{URL}/api/v1.0/device/eosdist1/generate_config", headers=AUTH_HEADER, verify=TLS_VERIFY)
        self.assertEqual(r.status_code, 200, "Failed to generate config for eosdist1")

    def test_08_plugins(self):
        r = requests.get(f"{URL}/api/v1.0/plugins", headers=AUTH_HEADER, verify=TLS_VERIFY)
        self.assertEqual(r.status_code, 200, "Failed to get running plugins")

        r = requests.put(f"{URL}/api/v1.0/plugins", headers=AUTH_HEADER, json={"action": "selftest"}, verify=TLS_VERIFY)
        self.assertEqual(r.status_code, 200, "Failed to run plugin selftests")

    def test_09_firmware(self):
        r = requests.get(f"{URL}/api/v1.0/firmware", headers=AUTH_HEADER, verify=TLS_VERIFY)
        # TODO: not working
        self.assertEqual(r.status_code, 200, "Failed to list firmware")

    def test_10_sysversion(self):
        r = requests.get(f"{URL}/api/v1.0/system/version", verify=TLS_VERIFY)
    ...
    def test_11_get_prev_config(self):
        ...
        if r.json()["data"]["failed"]:
            return
        data = {"job_id": prev_job_id, "dry_run": True}
        r = requests.post(
            f"{URL}/api/v1.0/device/{hostname}/previous_config", headers=AUTH_HEADER, verify=TLS_VERIFY, json=data
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_12_abort_running_job(self):
        data = {"group": "DIST", "url": "", "post_flight": True, "post_waittime": 30}
        result = requests.post(f"{URL}/api/v1.0/firmware/upgrade", headers=AUTH_HEADER, json=data, verify=TLS_VERIFY)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json()["status"], "success")
        self.assertEqual(type(result.json()["job_id"]), int)
        job_id: int = result.json()["job_id"]
        time.sleep(2)
        result = requests.get(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, verify=TLS_VERIFY)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json()["status"], "success")
        self.assertEqual(len(result.json()["data"]["jobs"]), 1, "One job should be found")
        self.assertEqual(
            result.json()["data"]["jobs"][0]["status"], "RUNNING", "Job should be in RUNNING state at start"
        )
        abort_data = {"action": "ABORT", "abort_reason": "unit test abort_running_job"}
        result = requests.put(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, json=abort_data, verify=TLS_VERIFY)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json()["status"], "success")
        result = requests.get(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, verify=TLS_VERIFY)
        self.assertEqual(
            result.json()["data"]["jobs"][0]["status"], "ABORTING", "Job should be in ABORTING state after abort action"
        )
        time.sleep(30)
        result = requests.get(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, verify=TLS_VERIFY)
        self.assertEqual(result.json()["data"]["jobs"][0]["status"], "ABORTED", "Job should be in ABORTED state at end")
    ...
    def test_13_abort_scheduled_job(self):
        ...
        self.assertEqual(len(result.json()["data"]["jobs"]), 1, "One job should be found")
        self.assertEqual(
            result.json()["data"]["jobs"][0]["status"], "SCHEDULED", "Job should be in SCHEDULED state at start"
        )
        abort_data = {"action": "ABORT", "abort_reason": "unit test abort_scheduled_job"}
        result = requests.put(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, json=abort_data, verify=TLS_VERIFY)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json()["status"], "success")
        result = requests.get(f"{URL}/api/v1.0/job/{job_id}", headers=AUTH_HEADER, verify=TLS_VERIFY)
