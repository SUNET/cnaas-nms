...
docker exec -u root -T cnaas_api chown -R www-data:www-data /opt/cnaas/venv/cnaas-nms/src/
docker exec -u root -T cnaas_api kill $MULE_PID
...
docker exec -u root -T cnaas_api chown -R root:www-data /opt/cnaas/venv/cnaas-nms/src/
